# Name:    BuildProfilerNETdll
# Purpose: Compile the mdProfilerNET.dll wrapper 
...
$Wrapper          = [FileConfig]@{
  FileName        = "mdProfiler_cSharpCode.cs";
...
Write-Host "`n======================= WELCOME TO MELISSA PROFILER OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdProfilerNET.dll -platform:AnyCPU -target:library mdProfiler_cSharpCode.cs" > $null

if (Test-Path ("mdProfilerNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdProfilerNET.dll"

  Write-Host "`nmdProfilerNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdProfilerNET.dll was not created successfully`n"
